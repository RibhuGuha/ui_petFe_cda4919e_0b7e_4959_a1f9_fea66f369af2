...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, petCareCenterId;
        const mapItems = MapItems;

        // Attach inline objects
        let petCareCenter = row['petCareCenter'];
        ...
        inlineObjs.forEach(x => {
            ...
            petcarecenter.push({ key: x.property, value: vObj, type: "inline" });
        });
        ...
        inlineObjs.forEach(x => {
            ...
            _values.forEach(m => {
                ['action', , 'id'].forEach(z => delete m[z]);
            });
            petcarecenter.push({ key: x.property, value: _values, type: "collections" });
        });


        // Add Or Update PetCareCenter
        rslt = await Support.AddOrUpdatePetCareCenter(petcarecenter, ['Logo',]);
        if (rslt.status) {
            petCareCenterId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the PetCareCenter and navigation entity if it is deos not exist
            let navItem = petcarecenter.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "PcId", value: parseInt(petCareCenterId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdatePetCareCenter(data);
        ...
        global.AlertPopup("success", "PetCareCenter is created successfully!");
        setShowUpdate(false);
        NavigateTo("/");
    }
